    ...
    <View style={[styles.screen, { paddingTop: insets.top }]}>
      ...
      <View style={styles.barWrap}>
        <View style={styles.bar}>
          ...
          <TextInput
            style={styles.input}
            placeholder="Rechercher des comptes ou posts…"
      ...
      {!loading && query.length < 2 && (
        <EmptyState
          icon="search-outline"
          title="Rechercher"
          subtitle="Trouvez des comptes et des publications"
        />
      )}
      ...
      {!loading && tab === 'users' && results.users.length > 0 && (
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            >
              <Avatar uri={item.profilePicture} username={item.username} size={48} />
            ...
            >
              ...
              {item.likesCount > 0 && (
                <View style={styles.overlay}>
                  ...
                  <Text style={styles.overlayTxt}>{formatCount(item.likesCount)}</Text>
                </View>
              )}
            </TouchableOpacity>
          )}
...
const styles = StyleSheet.create({
  ...
  barWrap:      { paddingHorizontal: SPACING.md, paddingVertical: SPACING.sm },
  bar: {
    flexDirection:    'row',
    alignItems:       'center',
    gap:              SPACING.sm,
  ...
  tabText:      { color: COLORS.textSub, fontSize: FONTS.sizes.sm, fontWeight: FONTS.weight.medium },
